...
IWLS is the computational method used to fit GLMs.
It’s essentially a clever way to approximate maximum likelihood estimation (MLE) using repeated weighted least squares steps.
...
<center>![](../images/IWLS_vs_OLS.gif)</center>
...
It measures the force required to embed a .44 in steel ball halfway into a sample of wood.
This ball diameter was chosen to produce a circle with an area of 100 square millimeters, or one square centimeter.

A common use of Janka hardness ratings is to determine whether a species is suitable for use as flooring.

![](../images/Janka_hardness_test.png) <br> <br> Based on the sampled species above it looks like the hardest Brazilian hardwoods are much harder than you might expect based on their ordering.
The pattern looks 'non-linear' and almost seems exponential at the bottom (hardest) end. Obvious curvature. 
<br> <br>
...
<div style="text-align: center;">
  <img src="images/interpret_coeff-01.png" width="400">
</div>
...
-   **Linearity** The response variable can be described by linear combination of the parameters (regression coefficients) and the predictor variables.

-   **Constant variance** (a.k.a. homoscedasticity).
    The variance of the errors (residuals) does not depend on the values of the predictor variables.

-   **Independence of errors** The errors of the response variables are uncorrelated with each other (when having more than one response variable).

-   Lack of strong **multicollinearity** in the predictors.
    Each predictor show provide independent information.
...
```{r check lm assumptions, message=FALSE, warning=FALSE, fig.height=8, fig.width=9}
...
One way (the traditional way) to deal with these issue is to transform the response variable.
But how should be transform the response?
What mathematical function should be use?
...
Box Cox transformations are done by raising each point of the response variable (y) to a power (lambda, $\lambda$) that is systematically varied (e.g. applying several transformations and selecting the best).
For example, when $\lambda$=2, the data are squared, when $\lambda$=0.5 they are square rooted, and so on.
In Box-Cox transformations, only the response variable (y) is transformed to improve model assumptions, while the predictor variables (x) remain unchanged.

```{r janka box cox, fig.align='center'}
...
```{r squared values, echo=FALSE, message=FALSE, warning=FALSE}

# Original values
original_values <- c(2.00, 5.64, 9.26, 14.26, 34.21)

# Create data frame with math-friendly column names
data <- data.frame(
  Untransformed = original_values,
  `$y^2$` = original_values^2,
  `$y^3$` = original_values^3,
  check.names = FALSE
)

# Display the table
knitr::kable(data, format = "pipe", digits = 2, align = "c",
      caption = "Original Data and Powers of y")
```
...
The transformation doesn't always work well, so make sure you check your data after the transformation with a [normal probability plot](https://en.wikipedia.org/wiki/Normal_probability_plot).
...
[Available online here](https://www.jstor.org/stable/2984418).
...
GLMs allow for response variables with non-normal distributions, making them flexible for ecological data.
The choice of error structure (distribution + link function) depends on the type of response variable:\
\
...
We can fit the same model using the equivalent GLM function:

```{r janka GLM}
janka.ml1 <- glm(Hardness~Density, family = gaussian(link = "identity"), data = janka)
anova(janka.ml1)
```
...
*Gaussian* is another word for 'normal'; the *identity* link is equivalent to performing no transformation (like a Box-Cox λ=1), so no transformation is applied to the fitted values from the linear predictor.
...
ggplot(janka,aes(Density,Hardness)) + 
  geom_point() +
  geom_smooth(method = "glm", method.args = list(family = "Gamma"(link = "sqrt")), colour="red")+
